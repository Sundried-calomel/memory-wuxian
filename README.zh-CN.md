...
> **2.19.2：**支持同一 Codex 对话延续到多个物理 rollout 文件。每个物理段拥有独立、
> 持久的采集游标和 Token 账本，但所有段仍归属于同一个顶层对话。Capture Core 同时识别
> 已观测到的 `item_completed` 可见消息封装，并只对实际使用该格式的保留来源执行一次有界、
> 幂等回填。归档水位必须覆盖每个物理段；最近更新时间不能代替完整性证明。
...
应用重建命令可先把旧派生文件保存在外置的 `backup.workspace_directory`；未配置时，默认使用实时 `memory` 根目录旁边的 `recovery-backups`。恢复副本绝不存放在实时归档内部，因此桌面快照不会再次复制它们。恢复副本使用 `backup.workspace_retention_count`，默认只保留最新一份。开发编辑使用一份可替换代码备份，不额外复制实时对话档案。
