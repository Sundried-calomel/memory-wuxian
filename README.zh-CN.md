...
> **2.19.3：** 合并 Mac 多段对话及 item_completed 采集与 Windows 语义任务隔离修复。源文件变化造成的恢复错误独立隔离，已排除的子代理游标可更新且不导入其内容。`source_reconcile.py --root ARCHIVE --source ROLLOUT` 预览经过原文核验的精确后缀对齐；仅通过核验的定位可加 `--apply` 应用。新增记录使用来源代次标识避免旧行号冲突。历史缺失、锚点不明确或 WAL 事务未解决时仍会阻止恢复，已归档原文始终保留。

> **2.19.2：**归档出现完整性告警时，不再冻结全部既有语义任务。Memory 无限仍会
> 显示告警，并禁止从不完整归档创建新任务；已经冻结且绑定来源哈希的任务则逐项验证、
> 独立处理。失败任务继续隔离，健康任务继续消化积压，全程不改写原始历史。

> **2.19.1：**修复语义维护吞吐与跨设备云同步闭环。每个维护批次只读取一次不可变
> 来源快照，并发执行模型任务，隔离失败任务，最后一次性提交派生索引更新。摘要重建
> 保留非连续轮次元数据；联邦同步会隔离发生漂移的摘要记录，并从已验证的不可变记录
> 重放，不复制原始对话或摘要 Markdown 正文。Archive、Environment、Evidence 与
> Attachment 流相互独立，成功的 ACK、导入和发布阶段会立即持久化。格式异常的 rollout
> 会按无正文指纹隔离，文件未变化时不再逐轮重复解析；它仍显示为覆盖债务，文件变化后自动重试。
> macOS 安装包同时恢复正式的通用架构操作台构建，并在事务回滚重试时分别保留不同的失败目录。
...
> 安装包哈希和签名更新元数据。原始历史、Summary V1/V2 行为、索引和云协议均不改变。
> 公开 CLI 的程序身份固定为 `memory-wuxian`，帮助与解析错误不会继承宿主进程的 `argv[0]`。

> **2.18.0：**新增有界且不含对话内容的 `capture-wal.jsonl`，用于幂等崩溃恢复；
> 同时加入来源级故障隔离、非致命遥测写入、纯读服务健康检查和维护失败非零退出。
> 原始历史、摘要、回填和云协议均不改变。

> **2.17.0：**将 P0 Capture Core 从产品逻辑中独立出来，并由薄型原生
> `memory-wuxian-core-launcher` 调用。
> 文件 watcher 会在有界启动追赶之前就绪；来源选择、可见事件归一化、归档字节、
> 游标及摘要资格均保持不变。
...
采集器在 `imports/codex/collector-telemetry.json` 发布轻量运行遥测。状态台显示活跃、空闲或深度空闲模式、当前补检间隔、最近文件事件、最近归档写入、一小时唤醒次数以及 CPU/内存。新进程先报告 `phase=starting` 和 `ready=false`，事件循环可用后进入 `phase=ready`，历史补采按有界批次继续；服务就绪不代表历史已全部归档。遥测在监测周期续写，分别记录来源水位与归档水位，只有完整来源覆盖检查通过才推进归档水位。启动未完成、遥测过期、采集器停止或历史仍落后时，状态台会明确告警。
