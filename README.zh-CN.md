...
在 macOS 上，OneDrive“文件按需”信封可能已经显示在目录中，但本地尚无可读字节。初次探测遇到这种占位文件时，Memory無限 会调用 OneDrive 支持的 `/pin` 命令，最多等待 120 秒直至字节可读，再执行 `/clearpin`，避免信封长期固定占用本机空间。暂时性的 File Provider 可用性错误仍作为可重试状态，而不是损坏文件。对于 `environment-v1`，如果发送端从更早游标重发了覆盖范围更大的包，只有在已持久化前缀逐事件完全一致时才会安全接续；任何冲突仍会失败关闭并隔离。
