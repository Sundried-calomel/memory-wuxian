...
### MW-REL-034：派生索引重建丢失来源字段并让单项失败阻断同层工作

- 证据：`本机历史样本与回归已核验`。重建摘要注册表时，旧实现只按当前解析结果重新计算
  `source_round_count`，没有保留摘要中已有的稀疏 `source_round_numbers`；同一份不可变摘要
  Markdown 因而产生不同的联邦 artifact hash，Windows 后续事件被首个漂移摘要阻断。语义维护
  同时为每个任务重复扫描全部原文和摘要，并在单项入库后立即重建全部派生索引；一个失败任务
  或一个失败云流会拖住同批其他独立工作。
- 根因：不可变内容、可重建投影与批次提交边界没有分离；派生字段被误当成可随时重新解释的
  数据，批次入口也没有共享一次只读快照和一次短提交。
- 逃逸边界：单个摘要生成、单流同步和小归档测试均可通过；只有历史稀疏轮次元数据、跨设备
  artifact hash 校验、生产规模批次及相邻失败任务同时出现时才会停摆或显著变慢。
- 永久门槛：重建派生索引必须保留原摘要可追溯字段并证明历史 artifact hash 恒等；无法合法
  重放的单个摘要写入隔离收据，不能跳过事件，也不能阻断独立流。每个维护批次只建立一次
  不可变来源快照，模型任务可并行，最后只进行一次有界派生索引提交；ACK、导入和发布进度在
  每阶段成功后分别持久化。
- 回归：使用真实历史摘要 `L1-001088` 验证哈希恒等；覆盖坏摘要与好摘要相邻、一个模型任务
  失败而兄弟任务成功、一个云流失败而其他流继续、批次单次扫描/单次 finalize，以及旧 CLI
  顶层 `acks/imports/published` 兼容合同。
- Families: `G03`, `G05`, `G06`, `G09`, `MW-R03`, `MW-R05`, `MW-R06`.

### MW-REL-035：异常 rollout 每轮重试阻断安装效果水位

- 证据：`本机生产故障与回归已核验`。4 个文件名与会话元数据不一致的 rollout 在启动时可被
  局部隔离，但事件循环仍每 5 秒重新解析；任一失败使整轮水位不提交，事务安装器因此无法在
  时限内确认已经成功写入的效果探针。
- 根因：来源隔离只有单次批次结果，没有持久化“这个精确文件版本已经失败”的派生状态；普通
  事件入口也仍采用首错即停语义。
- 逃逸边界：单次启动的 partial 结果和健康来源归档都能通过，但连续运行、安装效果探针和未变
  异常源同时出现时会形成无限重试。
- 永久门槛：按 canonical path、字节数和纳秒 mtime 持久化不含正文的来源故障指纹；未变来源
  必须跳过，发生变更必须自动重试，成功后清除指纹。异常来源始终计入覆盖债务，不能删除、
  改写或用隔离状态伪造完整水位；同批健康来源必须继续提交。
- 回归：覆盖异常与健康来源同批、下一轮跳过未变异常源、修改后恢复候选资格、成功后清除故障
...
- 证据：`本机事务失败与回归已核验`。上次切换中断后，活动候选目录因后续运行期状态与最初
  candidate generation 不再完全一致；恢复器只识别精确 candidate hash，随后直接把 prior
  generation `os.replace` 到仍非空的活动目录，macOS 返回 `Errno 66`。
- 根因：恢复合同把“候选已经切换”和“活动目录仍保持候选原始哈希”错误地绑定为同一条件，
  没有为切换后的合法漂移定义可恢复的 displaced evidence 路径。
- 逃逸边界：普通失败回滚、未变候选中断恢复和第二次幂等恢复均通过；只有中断后活动候选目录
  发生变化时触发。
- 永久门槛：恢复 prior 前，任何不是 prior generation 的活动 Skill 都必须先按其实际 tree hash
  原子移入失败证据目录；已有同名证据必须哈希一致，否则失败关闭。正式目标为空后才能恢复
  prior，第二次恢复必须无操作。
- 回归：在候选切换后写入运行期文件，验证实际代次进入 `recovery-displaced` 证据目录、prior
  精确恢复、rollback receipt 生成，并验证第二次恢复不再执行。
- Families: `G03`, `G05`, `G09`, `MW-R03`, `MW-R06`.

### MW-REL-037：临时单架构二进制进入正式操作台包且重复回滚冲突

- 证据：`本机安装故障与真实自检已核验`。仓库应用包中的 `MemoryDashboard` 被一次临时
  arm64 构建替换，未经过正式 universal app 构建脚本；该二进制在 macOS 上以成功状态
  静默退出 `--self-check`，安装器随后无法解析 JSON。回滚时已有同名失败候选证据，旧逻辑
  又直接报错，导致候选与 prior 都无法完成事务闭环。
- 根因：发布彩排检查了应用路径、签名和测试替身，却没有对仓库中实际打包二进制执行架构
  与真实输出验收；回滚还把一个 transaction 只有一个失败 tree 当成不变事实。
- 永久门槛：macOS 操作台只能由 `build_dashboard_app.sh` 生成，必须同时包含 arm64 与
  x86_64，并使用实际 launcher config 执行 `--self-check`、解析 `status=ok`。重复回滚若
  已有不同失败证据，必须按活动 tree 的实际 hash 保存到独立 displaced 路径；相同证据则
  幂等复用，不得覆盖或删除旧证据。
- 回归：验证 universal Mach-O、真实配置自检、已有冲突失败证据时的实际代次另存、相同
  失败证据幂等复用，以及 prior generation 精确恢复。
- Families: `G03`, `G05`, `G09`, `MW-R03`, `MW-R06`.

### MW-REL-038：摘要索引默认形状与分秒写入导致旧云账本无法重放

- 证据：`本机生产故障与精确哈希回归已核验`。`L1-001545` 的摘要 Markdown 与来源哈希
  完整，但创建摘要文件和追加索引分别调用时钟，索引的 `created_at` 比 Markdown 晚 1 秒；
  索引重建从 Markdown 恢复时间后，既有云账本绑定的摘要载荷无法复现。
- 根因：旧版联邦兼容器只在 `policy_events` 为空时尝试时间戳前后 1 秒，错误地把时间写入
  竞态与空策略默认值绑定；索引重建还把历史 L2/L3 的 `source_round_count: null` 或缺失值
  统一投影为 `0`，并补入空 `source_round_numbers`，但没有保留账本重放投影。
- 逃逸边界：摘要正文、来源、策略事件和索引字段均正确，普通检索与索引重建通过；仅在
  历史账本已登记、两次写入跨秒、摘要含非空策略事件且需要再次导出时触发。
- 永久门槛：历史摘要只可尝试 `created_at` 前后 1 秒，以及空默认字段的已知旧形状：
  `source_round_count` 的 `0`、`null` 或缺失和空 `source_round_numbers` 的保留或缺失；只有
  候选完整载荷 SHA-256 与既有账本精确相等时才允许重放并持久化紧凑记录。不得删除非空
  策略事件、修改账本、改写摘要正文或放宽真实漂移的失败关闭。
- 回归：构造含非空策略事件的跨秒摘要，以及父摘要的 `0/null/缺失` 旧默认投影，验证旧载荷
  精确恢复、策略事件完整保留、哈希一致；全量账本预检覆盖 1,728 条摘要并得到未解决为 0，
  不匹配候选仍由既有 typed failure 覆盖。
- Families: `G03`, `G05`, `G06`, `G09`, `MW-R03`, `MW-R05`, `MW-R06`.

### 2.20 候选分支编号合并说明

下列 7 条 Windows 安装器候选记录在未发布分支中曾暂用 `MW-REL-034` 至
`MW-REL-040`。合并已发布的 2.19.1 主线后，为保留主线已经公开的
`MW-REL-034` 至 `MW-REL-038`，现将候选记录顺延为 `MW-REL-039` 至
`MW-REL-045`。既有不可变收据继续保留原编号和原哈希，仅作为当时证据；不得据此
改写历史收据。

### MW-REL-039：采集器计划任务 XML 绕开共享身份 Owner
...
### MW-REL-040：事务探针嵌套完整目录树触发 Windows 长路径失败
...
### MW-REL-041：本机通过的字节收据与路径夹具在 CI Runner 漂移
...
### MW-REL-042：组合断言与原始恢复文件使安装失败既不可定位又泄露权限材料
...
### MW-REL-043：Unicode 快捷方式终态被 COM 直接复开时字段静默为空
...
### MW-REL-044：dirty-only 基线把正常提交误判成工作流漂移
...
### MW-REL-045：昂贵 S09 链未预取合同声明的历史标签
