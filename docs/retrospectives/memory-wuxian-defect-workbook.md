...
### MW-REL-016：定向发布合同声明了彩排场景但执行器未注册

- 症状：v2.14.5 合同列出四个 Project Evidence 必跑场景，但三平台 CI 都在开始
  定向彩排前以 `unknown contract rehearsal scenarios` 失败。
- 根因：版本合同和通用彩排目录分别维护场景 ID，新增合同只更新了前者；本地功能测试
  与真实发布证据均不能证明 CI 能解析并执行合同。
- 逃逸边界：候选提交前运行了底层 Python、Rust 与真实设备路径，却没有从当前版本合同
  调用一次 `run_release_rehearsal.py --contract-profile`。
- 永久门槛：每个定向合同中的场景 ID 必须在同一变更注册到彩排目录；候选 CI 必须从
  当前合同解析并执行全部场景。新增云流还要分别覆盖导出哈希、原生往返、跨流拒绝和
  通用云传输层的认证加密发布。
- Families: `G03`, `MW-R06`, `MW-R09`.
