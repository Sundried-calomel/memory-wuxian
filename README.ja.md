...
> **2.14.1:** macOS インストールを修正し、オフラインで分離された PyYAML フォールバックランタイムを追加します。**2.14.0** はデバイスローカルの Project Evidence Owner が、明示された閉じたファイル選択を限定的かつモデル非依存で更新します。**2.13.0** は明示的で不変なプロジェクト証拠パッケージと独立暗号化
