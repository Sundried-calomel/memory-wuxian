...
> **2.19.4 収集互換性修正：** 成功した `item_completed/FileChange` は旧パッチ表示を使用し、不正・失敗イベントを拒否します。照合済みの先頭部分を新しい ID で再取り込みしません。履歴計画は完全一致と出現回数で照合して既存レコードを保持し、欠落や未解決 WAL があれば通常の apply を拒否します。個人履歴の自動修復や要約の再生成は行いません。
