...
> `collector-lifecycle.json` に監査証拠を保持します。
> コレクター専用の有効化証明は `collector_runtime_effect_gate.py` が担当し、
> 既存の `runtime_effect_gate.py` の要約・保守チェックは変更しません。

> **2.15.0:** 独立して暗号化された `project-attachment-v1` 大容量添付ストリームを
> 追加します。正確な 4 MiB チャンク、SHA-256 マニフェスト、再開可能な転送、
> 検証済みのアトミック復元により、元ファイルの可読性と既存ストリームを維持します。

> **2.14.5:** Project Evidence のクラウド契約を完成させ、ストリームに束縛されたネイティブ暗号種別と転送互換の bundle ハッシュを追加し、ストリーム横断の復号拒否を必須検証します。

> **2.14.4:** 実行時コンテキストカプセルの読み込みを完全な読み取り専用かつ既読確認不要にし、最新の決定的テレメトリ遷移から更新を判定し、旧ACKコマンドを無書き込みの互換no-opとして残します。**2.14.3** はL2セマンティックジョブの再実行を修正し、5分ごとの限定バッチを8件に増やし、モデル呼び出しを最大3並列に限定しながらアーカイブ書き込みを直列化し、完全復旧監査を5分のホットパスから外し、操作台に段階別時間を表示し、トランザクション成功後にのみ旧macOSセマンティックバックフィル起動項目を廃止します。**2.14.2** は同一サイズの rollout 書き換えをバイトハッシュで検証し、macOS ユーザートランザクションがコレクターの準備完了待機時間を最後まで使えるようにします。**2.14.1** は macOS インストールを修正し、オフラインで分離された PyYAML フォールバックランタイムを追加します。**2.14.0** はデバイスローカルの Project Evidence Owner が、明示された閉じたファイル選択を限定的かつモデル非依存で更新します。**2.13.0** は明示的で不変なプロジェクト証拠パッケージと独立暗号化
...
Memory無限は代替タスクを新規作成せず、圧縮履歴を継続中のCodexタスクへ定期的に復元できます。`context-refresh-status`は最新の完了ラウンド節目、コンテキスト利用率がしきい値を越えた瞬間、または最新の圧縮イベントを検出します。更新が必要な場合、`context-capsule`は有用な最高レベルのセマンティック要約を選び、親に包含された子要約を除外し、少量の直近会話を追加して、安定した`refresh_id`付きの一時的な派生コンテキストを生成します。同じIDが現在の推論コンテキストにあれば読み込みを省略し、圧縮でIDが失われた後は再読み込みできます。読み込みは完全に読み取り専用で、既読確認も権限付き書き込みも不要です。非推奨の`ack-context-refresh`は旧呼び出し元向けの無書き込み互換no-opとしてのみ残り、通常運用では使用しません。
...
macOSでは、OneDrive Files On-Demandのエンベロープがディレクトリに表示されても、まだローカルで読めない場合があります。最初のプローブでこのプレースホルダーを検出すると、Memory無限はOneDriveがサポートする`/pin`を実行し、読取可能になるまで最大120秒待機してから`/clearpin`を実行し、エンベロープを永続的に固定しません。一時的なFile Provider可用性エラーは破損ではなく再試行可能として扱います。`environment-v1`で送信側が過去のカーソルから広い範囲を再送した場合、永続化済みの接頭イベントがすべて完全一致するときだけ安全に続行し、競合する重複は引き続き失敗閉鎖で隔離します。
...
大きな最終成果物は Project Evidence の上限を引き上げず、独立した
`project-attachment-v1` に格納します。明示的に閉じた JSON 仕様では PDF、
PPTX、DOCX、XLS/XLSX、TIF/TIFF、PNG、JPEG、WebP を選択できます。
元ファイルは変更せず、4 MiB チャンクと完全ファイルの SHA-256 を記録します。
1 ファイルは 256 MiB、1 世代は 1 GiB が上限です。

暗号化転送は再開可能でストリームに束縛されます。全チャンクと完全ファイルの
ハッシュが通るまで復元先には何も生成しません。成功時のみ検証済み復元受領書を
記録し、欠落、破損、順序違反、誤った宛先、ストリーム横断は閉じて失敗します。
添付だけを許可する場合は専用同期コマンドを使います。詳細は
[プロジェクト添付契約](references/project-attachments.md) を参照してください。
