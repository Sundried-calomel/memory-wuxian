...
class DashboardShortcutTest(unittest.TestCase):
    ...
    def test_unicode_shortcut_round_trips_through_canonical_inspector(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertTrue(Path(document["target"]).samefile(launcher))
            self.assertTrue(Path(document["working_directory"]).samefile(skill))
            icon_path, icon_index = document["icon"].rsplit(",", 1)
            self.assertTrue(Path(icon_path).samefile(icon))
