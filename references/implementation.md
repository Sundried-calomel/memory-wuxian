...
Automatic semantic-job creation is enabled in the installed configuration. After a due job is frozen at a completed-round boundary, invoke one ephemeral Codex CLI process for constrained summary JSON, ingest it after source-hash verification, and exit. One maintenance batch may overlap no more than three independent model calls. Source-hash verification, archive ingestion, parent-job creation, and state/index mutation remain serialized through their existing locks. Existing pending jobs retain their original source ranges and hashes.
...
The hidden five-minute maintenance supervisor reconciles all pending summary jobs and backup debt in linear time and writes a bounded dashboard projection. It continues model-free mechanical and backup work while Codex is closed. A clean full-recovery audit is reusable for 24 hours; explicit recovery debt or an expired audit forces a new full check. The routine batch selects at most eight jobs and may overlap at most three model calls, while verified archive ingestion remains serialized. One job failure is isolated and does not cancel successful siblings. Semantic unavailability defers a job without consuming a retry and does not block later eligible jobs. Persist only aggregate phase timing for recovery, local preparation, model execution, validation, ingestion, and total batch duration; never persist prompts or source text as telemetry. An oversized summary uses a hash-bound resumable map-reduce plan: complete rounds are preferred, an oversized round falls back to contiguous record groups, and an oversized record falls back to bounded field spans. Every actual prompt satisfies both the configured character and UTF-8 byte limit, completed pieces are reused only after hash verification, and the original parent job is the only object ingested as the final Level-1 summary.
...
Keep context refresh stateless and derived; it is not authoritative conversation history and does not require an archive snapshot. At the start of an Agent turn, inspect the newest top-level rollout and mark refresh due only when the latest state lands on the configured completed-round milestone, the latest `token_count` crosses 65% or 80%, or the latest usage falls by at least 20 percentage points after reaching the low threshold, which indicates client compaction. Derive a stable `refresh_id` from that event and skip it when the same ID is already present in the active reasoning context.

Render a capsule from the highest available semantic summary levels, omit child summaries already covered by a selected parent, append uncovered newer summaries, and finish with a bounded recent-task tail. Cap the estimated budget at the smallest of 1% of the rollout-reported effective context window, 3,000 tokens, and 10,000 tokens. Loading the capsule is a read-only, idempotent context operation. It requires no acknowledgement, may not block on a write permission, and must never be appended to raw history or submitted as semantic-summary source. Keep `ack-context-refresh` only as a deprecated no-op for older callers.
