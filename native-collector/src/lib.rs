...
use runtime::{ACTIVE_FALLBACK, adaptive_fallback};
use source::is_rollout_file;
use store::cursor::{
...
use telemetry::CollectorTelemetry;
...
impl Store {
    ...
    fn lock<T>(&self, name: &str, operation: impl FnOnce() -> Result<T>) -> Result<T> {
        ...
        locking::with_exclusive_lock(&path, operation)
    }
...
#[cfg(not(target_os = "macos"))]
fn prepare_watcher(sessions_root: &Path) -> Result<PreparedWatcher> {
...
fn run() -> Result<()> {
    ...
    let prepared_watcher = if args.once {
        ...
        Some(KqueueWatcher::new(&sessions_root, since)?)
    };
    #[cfg(not(target_os = "macos"))]
    let prepared_watcher = if args.once {
        None
    } else {
        Some(prepare_watcher(&sessions_root)?)
    };
    if !args.once {
    ...
    run_event_loop(
        ...
        since,
        args.debounce_ms,
        prepared_watcher.expect("continuous collector prepares a watcher"),
        startup_telemetry,
    )
}

pub fn run_in_thread() -> Result<()> {
